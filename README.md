...
protocol, seeded with `--seed 0`; full report in `results/eval_flower.json`,
regenerated exactly by `python -m morphogenesis eval`):

| protocol | SSIM vs. target (mean) | min over 10 trials |
|---|---|---|
| grow from a **single seed** (96 steps) | **0.965** | 0.961 |
| destroy **80% of living cells at random** → 200 recovery steps | **0.960** | 0.955 |
| destroy 80%, **one contiguous ~20% fragment survives** → 300 steps | **0.955** | 0.940 |
...
(`results/eval_goose.json`: growth 0.990, 80% random destruction 0.992,
fragment-only 0.985 mean SSIM over 5 trials):
